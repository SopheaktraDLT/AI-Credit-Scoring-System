...
def render_assessment_history():
    ...
    if db.connect():
        try:
            ...
            query = """
            SELECT
                ah.assessment_id as assessment_id,
                CONCAT('LN', LPAD(ah.assessment_id, 5, '0')) AS loan_id,
                u.user_id,
                CASE
                    WHEN ah.assessment_id = (
                        SELECT MIN(ah2.assessment_id)
                        FROM assessment_history ah2
                        WHERE ah2.user_id = u.user_id
                    )
                ...
                ah.final_credit_score,
                ah.risk_level,
                ah.recommendation AS approval_status,
                ah.default_probability
            FROM assessment_history ah
            LEFT JOIN users u 
                ON ah.user_id = u.user_id
            ORDER BY ah.assessment_id DESC
            """
            ...
            # Handle potential null values in default_probability column, ensuring consistent formatting
            ...
            with col4:
                st.metric(
                    "Application Approved", len(df[df["Approval Status"] == "APPROVE"])
                )
            with col5:
                st.metric(
                    "Application Rejected", len(df[df["Approval Status"] == "REJECT"])
                )
