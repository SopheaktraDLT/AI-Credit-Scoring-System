...
from src.scoring.recommendation import generate_recommendations
...
from src.utils.nid_verification import verify_nid
...
with st.sidebar:
            ...
            if db.connect():
                try:
                    ...
                    if existing_user:
                        ...
                        st.session_state["dob"] = existing_user.get("dob", "")
                        st.session_state["province_name"] = existing_user.get(
                            "geo_province", ""
                        )
                        ...
                        voter = verify_nid(n_id.strip())

                        if voter is not None:
                            full_name = voter.get("name", "")
                            ...
                            gender_mapping = {"ប្រុស": "Male", "ស្រី": "Female"}
                            ...
                            st.session_state["gender"] = gender_mapping.get(
                                voter.get("gender"), "Male"
                            )
                            st.session_state["dob"] = voter.get("dob", "")
                            st.session_state["province_name"] = voter.get(
                                "province_name", ""
                            )
                            st.session_state["nid_status"] = (
                ...
                st.error(
                    "NID not found on the voter list. Please verify the NID and try again."
                )
                st.stop()
    ...
    with col1:
        last_name = st.text_input(
            "Last Name", value=st.session_state.get("last_name", ""), disabled=True
        )
    with col2:
        first_name = st.text_input(
            "First Name", value=st.session_state.get("first_name", ""), disabled=True
        )
    col1, col2 = st.columns(2)
    with col1:
        gender = st.text_input(
            "Gender", value=st.session_state.get("gender", "Male"), disabled=True
        )
    with col2:
        dob = st.text_input(
            "Date of Birth", value=st.session_state.get("dob", ""), disabled=True
        )
    # Auto-calculate age from DOB
    age = 18
    dob_str = st.session_state.get("dob", "")
    ...
    province = st.text_input(
        "Province", value=st.session_state.get("province_name", "")
    )
...
current_user_type = st.session_state.get("user_type", "new")
...
if predict_button:
    ...
    try:
        ...
        probability, label = predict_default(
            input_data, current_user_type, model_choice, threshold
        )
        ...
        with col3:
            ...
            detailed_breakdown = calculate_score_breakdown(
                input_data, current_user_type
            )
            recommendation_df = generate_recommendations(detailed_breakdown)
            fico_score = final_credit_score_from_breakdown(
                detailed_breakdown, current_user_type
            )
            st.metric(
                "Overall Scoring",
        ...
        # Recommendation generation and presentation blocks based on low-scoring features identified in the breakdown analysis
        st.subheader("📈 Recommendations for Improvement")
        if recommendation_df.empty:
            st.success("""
                **Excellent Credit Profile! 🎉**
                
                No critical improvement areas were identified based on the current assessment.
                
                To maintain or further improve your credit standing, consider:
                • Continue making repayments on time.
        ...
        with col1:
            st.info(
                f"- **User Type:** {current_user_type.upper()}\n- **Model:** {model_choice}\n- **Threshold:** {threshold:.2f}"
            )
        with col2:
            st.info(
        ...
        try:
            ...
            if db.connect():
                ...
                if saved:
                    ...
                    user = db.get_user_by_nid(n_id.strip())
                    if user is not None:
                        history_saved = db.add_assessment_history(
                            user_id=user["user_id"],
                            model_used=model_choice,
                            score=fico_score["fico_score"],
